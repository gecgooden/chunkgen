			...
			while (chunkQueue > 1) {
				...
				if (cp != null) {
					try {
						...
						Reference.logger.error("Error generating chunks", e);
						ChatComponentTranslation chatTranslation = new ChatComponentTranslation("commands.errorGeneratingChunks", e.getLocalizedMessage());
						MinecraftServer.getServer().addChatMessage(chatTranslation);
						cp.getICommandSender().addChatMessage(new ChatComponentText(chatTranslation.getUnformattedTextForChat()));
						return;
					}
